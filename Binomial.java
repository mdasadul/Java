...
public class Binomial {
    ...
    static double[][] precompute;
    // slow
    public static double binomial1(int N, int k, double p) {
        counter ++;
        if (N == 0 && k == 0) return 1.0;
        if (N < 0 || k < 0) return 0.0;
        if(precompute[N][k+1]==-1){
            precompute[N][k+1] =binomial1(N-1, k, p);
        }
        if(precompute[N][k] ==-1)
            precompute[N][k]= binomial1(N-1, k-1, p);
        return (1.0 - p) * precompute[N][k+1]+ p*precompute[N][k];
        //return (1.0 - p) *binomial1(N-1, k, p) + p*binomial1(N-1, k-1, p);
    }
    private static void setPreCompute(int N,int k){
        precompute = new double[N+1][k+2];
        for(int i =0 ;i<= N;i++)
        {
            for (int j =0;j<= k+1;j++)
            {
